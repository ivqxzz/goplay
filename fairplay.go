package main

import (
	"fmt"
	"log"
)

var fpReplyMessages = [4][142]byte{
	{
		,
	},
	{
		,
	},
	{
		,
	},
	{
		,
	},
}

var fpHeader = []byte{0x46, 0x50, 0x4c, 0x59, 0x03, 0x01, 0x04, 0x00, 0x00, 0x00, 0x00, 0x14}

var fpKeyMsg []byte

func initFairplay() {
	fpKeyMsg = nil
}

func fairplaySetup(body []byte) ([]byte, error) {
	switch len(body) {
	case 16:
		if body[4] != 0x03 {
			return nil, fmt.Errorf("fp-setup: unsupported FairPlay version %#x", body[4])
		}
		mode := int(body[14])
		if mode < 0 || mode > 3 {
			return nil, fmt.Errorf("fp-setup: invalid mode=%d", mode)
		}
		res := make([]byte, 142)
		copy(res, fpReplyMessages[mode][:])
		log.Printf("fp-setup phase 1: mode=%d -> 142-byte response", mode)
		return res, nil

	case 164:
		if body[4] != 0x03 {
			return nil, fmt.Errorf("fp-setup: unsupported FairPlay version %#x", body[4])
		}
		fpKeyMsg = make([]byte, 164)
		copy(fpKeyMsg, body)

		res := make([]byte, 0, 32)
		res = append(res, fpHeader...)
		res = append(res, body[144:164]...)
		log.Printf("fp-setup phase 2: keymsg(164) stored -> 32-byte response")
		return res, nil

	default:
		return nil, fmt.Errorf("fp-setup: unexpected body length %d bytes", len(body))
	}
}
